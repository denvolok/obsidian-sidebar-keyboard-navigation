...
export abstract class CommonActions {
	...
	public toggleHelpModal(keysHelp: { key: string; action: string }[]): void {
		...
		if (modalNode != null) {
			...
			const elTable = createEl("table");

			elContainer.appendChild(elTable);

			const elThead = createEl("thead");
			const elTbody = createEl("tbody");

			elTable.append(elThead, elTbody);

			const th1 = createEl("th");
			const th2 = createEl("th");
			th1.textContent = "Key";
